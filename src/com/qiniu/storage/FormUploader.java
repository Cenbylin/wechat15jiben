...
public final class FormUploader {
	...
	FormUploader(Client client, String upToken, String key, byte[] data,
			StringMap params, String mime, boolean checkCrc) {
		this(client, upToken, key, data, null, params, mime, checkCrc);
	}

	FormUploader(Client client, String upToken, String key, File file,
			StringMap params, String mime, boolean checkCrc) {
		this(client, upToken, key, null, file, params, mime, checkCrc);
	}

	private FormUploader(Client client, String upToken, String key,
			byte[] data, File file, StringMap params, String mime,
			boolean checkCrc) {
		this.client = client;
		token = upToken;
		this.key = key;
	...
	Response upload() throws QiniuException {
		buildParams();
		if (data != null) {
			return client.multipartPost(Config.zone.upHost, params, "file",
					fileName, data, mime, new StringMap());
		}
		return client.multipartPost(Config.zone.upHost, params, "file",
				fileName, file, mime, new StringMap());
	}

	void asyncUpload(final UpCompletionHandler handler) throws IOException {
		buildParams();
		if (data != null) {
			client.asyncMultipartPost(Config.zone.upHost, params, "file",
					fileName, data, mime, new StringMap(), new AsyncCallback() {

...
/**
 * 计算文件内容或者二进制数据的etag, etag算法是七牛用来标志数据唯一性的算法。 文档：<a
 * href="https://github.com/qiniu/qetag">etag算法</a>
 */
public final class Etag {
	...
	public static String data(byte[] data, int offset, int length) {
		try {
			return stream(new ByteArrayInputStream(data, offset, length),
					length);
		} catch (IOException e) {
	...
	public static String stream(InputStream in, long len) throws IOException {
		...
		byte[][] blocks = new byte[(int) (len + Config.BLOCK_SIZE - 1)
				/ Config.BLOCK_SIZE][];
	 ...
	 *             读取出错
	 */
	private static byte[] oneBlock(byte[] buffer, InputStream in, int len)
			throws IOException {
		MessageDigest sha1;
		try {
